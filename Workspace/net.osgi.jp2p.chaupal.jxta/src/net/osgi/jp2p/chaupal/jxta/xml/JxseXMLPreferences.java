 ...
 *******************************************************************************/
package net.osgi.jp2p.chaupal.jxta.xml;
...
import net.osgi.jp2p.chaupal.xml.PersistentAttribute;
import net.osgi.jp2p.chaupal.xml.PreferenceStore;
...
public class JxseXMLPreferences extends AbstractJp2pPropertySource implements IJp2pPropertySource<IJp2pProperties> {
	...
	protected static Map<SupportedAttributes, String> parseAttributes( Attributes attrs ){
		...
		for( int i=0; i<attrs.getLength(); i++ ){
			...
			case PERSIST:
				...
				PreferenceStore.Persistence.valueOf( value);
	 ...
	 * @return
	 */
	static PreferenceStore.Persistence isPersistentProperty( Attributes attrs ){
		if(( attrs == null ) || ( attrs.getLength() == 0 ))
			return PreferenceStore.Persistence.NULL;
		Map<SupportedAttributes, String> sa =  parseAttributes( attrs );
		if( sa.get( SupportedAttributes.PERSIST ) == null )
			return PreferenceStore.Persistence.NULL;
		return PreferenceStore.Persistence.valueOf( sa.get( SupportedAttributes.PERSIST ));
	}
