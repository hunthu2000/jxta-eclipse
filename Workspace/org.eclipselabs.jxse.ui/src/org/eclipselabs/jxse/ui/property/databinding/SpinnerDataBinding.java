...
public class SpinnerDataBinding<T extends Enum<T>> extends AbstractManagedPropertySourceDatabinding<T, Integer>{
	...
	public SpinnerDataBinding( T id, IJxsePropertySource source, Spinner spinner, int min, int max) {
		this(( ManagedProperty ) source.getManagedProperty(id), spinner, min, max);
	}

	@SuppressWarnings({ "unchecked", "rawtypes" })
	public SpinnerDataBinding( T id, IJxseWritePropertySource source, Spinner spinner, T id_min, T id_max ) {
